...
export function aggregateDropboxSize({ bucket, name = '', size }: ObjectMetadata, action: 'upload' | 'delete') {
    ...
    if (bucket !== 'gw-enterprise.appspot.com') return;
    // My only focal point is the file object event to the directory: faculty_${facultyId}/repo_${repoId}/dropbox_${userId}
    if (!/faculty_.+\/repo_.+\/dropbox_.+/.test(name)) return;
